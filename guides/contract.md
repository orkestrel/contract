...
| `isGeneratorFunction`      | function | generator function                | Determines whether a value is a generator function (`function*`).                                            |
| `isAsyncGeneratorFunction` | function | async generator function          | Determines whether a value is an async generator function (`async function*`).                               |
| `isZeroArgAsync`           | function | `ZeroArgAsyncFunction`            | Determines whether a value is a zero-argument async function.                                                |
| `isZeroArgGenerator`       | function | zero-arg generator function       | Determines whether a value is a zero-argument generator function.                                            |
| `isZeroArgAsyncGenerator`  | function | zero-arg async generator function | Determines whether a value is a zero-argument async generator function.                                      |
...
| `isJSONValue`         | function  | Determines whether a value is a cycle-safe JSON value.                                                                                      |
| `isBoundedJSONValue`  | function  | Determines whether a value is JSON-valid within the fixed container-depth limit.                                                            |
| `isBoundedJSONRecord` | function  | Determines whether a value is a depth-bounded JSON record.                                                                                  |
| `parseJSON`           | function  | Parses a JSON string, returning `undefined` instead of throwing.                                                                            |
...
| `JSONPrimitive`       | type      | Represents a primitive JSON value — the flat leaf of any JSON document.                                                                     |
| `JSONRecord`          | type      | Represents a readonly string-keyed JSON object record.                                                                                      |
| `JSONValue`           | type      | Represents a recursive JSON value — primitives, arrays, and object records.                                                                 |
| `JSONSchemaType`      | type      | Lists the seven standard JSON Schema `type` names.                                                                                          |
| `JSONSchema`          | interface | Represents a JSON Schema fragment — the supported keyword vocabulary the contract compiler emits and `RawShape` validates before embedding. |
| `SchemaFormat`        | type      | Lists the closed set of string formats `stringToFormat` recognizes.                                                                         |
...
| ------------------------ | --------- | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `Failure`                | interface | `{ success: false, error: E }`                                                                                                   | Represents the discriminated failure branch of a `Result`.                                                                                     |
...
| `Success`                | interface | `{ success: true, value: T }`                                                                                                    | Represents the discriminated success branch of a `Result`.                                                                                     |
...
| API                    | Kind      | Summary                                                                  |
| ---------------------- | --------- | ------------------------------------------------------------------------ |
| `isContractError`      | function  | Checks whether an unknown value is a `ContractError`.                    |
| `ContractCode`         | type      | Names the machine-readable category carried by a `ContractError`.        |
| `CONTRACT_CODES`       | const     | Lists every declared `ContractCode` refusal category, frozen.            |
| `ContractErrorContext` | interface | Represents the optional structured details carried by a `ContractError`. |
| `ContractErrorOptions` | interface | Represents the construction options for a `ContractError`.               |
...
| `JSONClonerInterface`   | interface | Owns the state of one exact JSON snapshot operation.     |
| `SchemaCloner`          | class     | Owns the state of one JSON Schema snapshot operation.    |
| `SchemaClonerInterface` | interface | Owns the state of one JSON Schema snapshot operation.    |
| `ShapeCloner`           | class     | Owns the state of one contract-shape snapshot operation. |
| `ShapeClonerInterface`  | interface | Owns the state of one contract-shape snapshot operation. |
...
| `StringShape`         | interface | `{ category: 'string', min?, max?, pattern?, … }`                                                                                                                               | Describes a string with optional length and pattern constraints.                                                                                                                                                  |
| `NumberShape`         | interface | `{ category: 'number', min?, max?, integer?, … }`                                                                                                                               | Describes a number with optional bounds; `integer` restricts to whole numbers.                                                                                                                                    |
| `BooleanShape`        | interface | `{ category: 'boolean', … }`                                                                                                                                                    | Describes a boolean — accepts only `true` or `false`.                                                                                                                                                             |
| `NullShape`           | interface | `{ category: 'null', … }`                                                                                                                                                       | Describes a null value — accepts only `null`.                                                                                                                                                                     |
| `LiteralShape`        | interface | `{ category: 'literal', values, … }`                                                                                                                                            | Describes a literal — accepts exactly one of a fixed set of primitive values.                                                                                                                                     |
| `ArrayShape`          | interface | `{ category: 'array', items, min?, max?, … }`                                                                                                                                   | Describes an array with an element shape and optional length bounds.                                                                                                                                              |
| `ObjectShape`         | interface | `{ category: 'object', properties: P, additionalProperties?: A, … }`                                                                                                            | Describes an object — a map of property names to child shapes.                                                                                                                                                    |
| `UnionShape`          | interface | `{ category: 'union', variants, mode?, … }`                                                                                                                                     | Describes a union — accepts a value matching any one variant (first match wins).                                                                                                                                  |
| `OptionalShape`       | interface | `{ category: 'optional', inner }`                                                                                                                                               | Wraps an inner shape that may be absent (`undefined`).                                                                                                                                                            |
| `NullableShape`       | interface | `{ category: 'nullable', inner }`                                                                                                                                               | Wraps an inner shape that may be `null`.                                                                                                                                                                          |
| `JSONShape`           | interface | `{ category: 'json', … }`                                                                                                                                                       | Describes a JSON passthrough — accepts any JSON value.                                                                                                                                                            |
| `RawShape`            | interface | `{ category: 'raw', schema }`                                                                                                                                                   | Describes a validated raw JSON Schema passthrough — embeds a supported schema fragment directly.                                                                                                                  |
...
| API                         | Kind      | Summary                                                                                                                                                                                    |
| --------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ShapeValidator`            | class     | Validates one retained contract-shape source live on every call.                                                                                                                           |
| `ShapeValidatorInterface`   | interface | Validates one retained contract-shape source on demand.                                                                                                                                    |
| `validateShape`             | function  | Gates recursive compiler work on shape structure, depth, and cycles.                                                                                                                       |
| `compileGuard`              | function  | Compiles a `ContractShape` into a runtime type guard.                                                                                                                                      |
| `compileParser`             | function  | Compiles a `ContractShape` into an input parser.                                                                                                                                           |
| `compileSchema`             | function  | Compiles a `ContractShape` into a JSON Schema document.                                                                                                                                    |
| `compileGenerator`          | function  | Compiles a `ContractShape` into a deterministic seed value.                                                                                                                                |
| `createContract`            | function  | Compiles a `ContractShape` into a `ContractInterface` — the lockstep outputs from one declaration, lockstep meaning derived from one owned snapshot rather than accepting the same values. |
| `ContractInterface`         | interface | Represents a compiled contract — the lockstep outputs derived from one shape.                                                                                                              |
| `ContractCompiler`          | class     | Owns one contract shape's artifacts and their bundle, compiled lazily.                                                                                                                     |
| `ContractCompilerInterface` | interface | Owns one contract shape's compiled artifacts plus their bundle, lazily.                                                                                                                    |
| `RandomFunction`            | type      | Represents a deterministic random source returning a value in `[0, 1)`.                                                                                                                    |
| `AuditorFunction`           | type      | Represents a compiled strict-domain diagnostic — the shape of `compileAuditor` bound to one shape.                                                                                         |
| `ReporterFunction`          | type      | Represents a compiled coercive-domain diagnostic — the shape of `compileReporter` bound to one shape.                                                                                      |
| `SeederFunction`            | type      | Represents a compiled seed-data source — the shape of `compileGenerator` bound to one shape.                                                                                               |
| `GENERATION_ATTEMPT_LIMIT`  | const     | Caps at `32` the number of candidate-generation attempts for a constrained generated value, frozen.                                                                                        |
| `COMPILE_DEPTH_LIMIT`       | const     | Caps at `512` the supported nesting depth of a compiled contract shape, frozen.                                                                                                            |
| `COMPILE_NODE_LIMIT`        | const     | Caps at `16384` the number of nodes a compiled artifact may expand a shape into, frozen.                                                                                                   |
| `PRESENCE_MASK_LIMIT`       | const     | Caps at `31` the number of object keys one compiled presence mask carries, frozen.                                                                                                         |
...
| API                    | Kind      | Summary                                                                                                                                                                           |
| ---------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `valueToSchema`        | function  | Infers a `JSONSchema` for one unknown value — the reverse direction of `compileSchema`.                                                                                           |
| `samplesToSchema`      | function  | Infers a `JSONSchema` from a set of example values — the multi-example counterpart of `valueToSchema` (for example inferring one schema from several database rows).              |
| `unifySchemas`         | function  | Unifies a list of inferred `JSONSchema` fragments into one schema.                                                                                                                |
| `canonicalStringify`   | function  | Renders a value as a deterministic, key-sorted JSON string — or `undefined` when it has no faithful JSON encoding.                                                                |
| `encodeLeaf`           | function  | Encodes one non-container value the way JSON encodes it, or `undefined` when JSON cannot encode it at all.                                                                        |
| `buildSampleMemo`      | function  | Builds one empty `SampleMemo` node.                                                                                                                                               |
| `readSampleMemo`       | function  | Checks that a value really is a `SampleMemo` before a walk stores a published schema in it.                                                                                       |
| `inferPrimitiveEnum`   | function  | Infers an `{ enum: [...] }` fragment for a low-cardinality, repeated primitive slot — the multi-sample-only counterpart to `stringToFormat` (`valueToSchema` never emits `enum`). |
| `stringToFormat`       | function  | Classifies a string against the `SchemaFormat` vocabulary.                                                                                                                        |
| `classifyFormat`       | function  | Classifies an already-bounded string against the pattern-only and calendar-checked `SchemaFormat` vocabulary.                                                                     |
| `samplesToFormat`      | function  | Classifies a list of sample values against the `SchemaFormat` vocabulary, requiring unanimity.                                                                                    |
| `matchesISOInstant`    | function  | Checks whether a supported ISO-8601 date or date-time names a real instant.                                                                                                       |
| `ValueToSchemaOptions` | interface | Groups the options for `valueToSchema` / `samplesToSchema`.                                                                                                                       |
| `ValueToSchemaLimits`  | interface | Holds the per-walk budgets `ValueToSchemaOptions` groups under `limits`.                                                                                                          |
| `SampleMemo`           | interface | Holds the per-walk memo the multi-sample walk behind `samplesToSchema` owns, keyed by the ORDERED identities of the rows a slot collected.                                        |
| `sanitizeDepth`        | function  | Resolves a caller's depth budget to one the traversal can actually survive.                                                                                                       |
| `sanitizeBudget`       | function  | Sanitizes a user-supplied inference budget (`limits.depth` / `limits.properties`) to a finite non-negative integer, selecting a valid fallback for anything else.                 |
| `INFER_DEPTH_LIMIT`    | const     | Caps at `32` the object/array nesting depth `valueToSchema` walks, frozen.                                                                                                        |
| `INFER_BREADTH_LIMIT`  | const     | Caps by default at `256` the number of object properties / array elements `valueToSchema` samples per container, frozen.                                                          |
| `INFER_ENUM_LIMIT`     | const     | Caps by default at `12` the number of distinct values a multi-sample slot may hold before enum inference gives up and falls back to a bare `type`, frozen.                        |
| `FORMAT_MAX_LENGTH`    | const     | Caps at `128` the string length `stringToFormat` attempts to classify, frozen.                                                                                                    |
| `FORMAT_PATTERNS`      | const     | Holds the pure-regex matchers backing `stringToFormat`'s pattern-only formats (`uuid` / `email` / `uri`), frozen as data.                                                         |
...
| API                   | Kind      | Summary                                                                                                                                 |
| --------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `compileAuditor`      | function  | Audits a value against the strict acceptance domain of a `ContractShape`.                                                               |
| `compileReporter`     | function  | Compiles a `ContractShape` into a structured fault report for a value — the diagnostic counterpart of `compileGuard` / `compileParser`. |
| `buildStringFaults`   | function  | Builds the refinement faults a string value has against a `StringShape`.                                                                |
| `buildNumberFaults`   | function  | Builds the refinement faults a number value has against a `NumberShape`.                                                                |
| `buildArrayFaults`    | function  | Builds the length faults an array has against an `ArrayShape`.                                                                          |
| `selectClosestFaults` | function  | Selects the report of the variant that came closest to matching.                                                                        |
| `shapeToKind`         | function  | Projects a `ContractShape` to the `FaultKind` it describes.                                                                             |
| `preview`             | function  | Renders an unknown value as a short, safe, TOTAL string for a `Fault`'s `received` field.                                               |
| `FAULT_LIMIT`         | const     | Caps at `64` the number of `Fault` / `AuditFault` entries a single `explain` or `audit` report ever returns, frozen.                    |
| `PREVIEW_LIMIT`       | const     | Caps at `64` the character length of a `preview`-rendered string, frozen.                                                               |
| `Fault`               | type      | Represents a single structured parse-failure diagnostic — one entry of an `ContractInterface.explain` report.                           |
| `ExtraFault`          | interface | Represents a key present on a value that its closed object shape does not declare.                                                      |
| `AuditFault`          | type      | Covers every fault an audit reports — the parse faults plus undeclared keys.                                                            |
| `FaultKind`           | type      | Names the kind of value a `Fault` expected — the shape-projected counterpart of a `ContractShape`'s `category`.                         |
| `FaultConstraint`     | type      | Names the refinement a `Fault` of reason `'constraint'` violates.                                                                       |
