...
| API                    | Kind      | Summary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `valueToSchema`        | function  | value + optional `ValueToSchemaOptions` → `JSONSchema`. Total: never throws, cycle/depth/breadth-bounded. Leaf order: `null`, boolean, integer (`Number.isInteger` semantics — `-0` counts), finite non-integer number, non-finite number (widened to `{ type: 'number' }`), string, array (recurse), plain record (recurse); anything else (function, symbol, bigint, `undefined`, `Date`/`Map`/`Set`, …) → the empty accept-anything schema `{}`. Nested objects close to unknown keys by default (`additionalProperties: false`); pass `closed: false` to open them. Deterministic — sorted property keys and sorted `anyOf` members. |
| `samplesToSchema`      | function  | samples (`readonly unknown[]`) + optional `ValueToSchemaOptions` → `JSONSchema` — multi-example inference (e.g. several database rows). An empty array infers `{}`. When every sample is a plain record, a key is `required` only when present (and non-`undefined`) in EVERY sample, and `properties` is the union of every sample's keys; the per-key schema unifies across all samples' values for that key. Otherwise each sample is inferred independently via `valueToSchema` and the results are unified the same way heterogeneous array elements are.                                                                           |
...
| `inferValue`           | function  | value + depth + breadth + `closed` + a `WeakSet` ancestor set → `JSONSchema` — the recursive spine `valueToSchema` and the container inferers share; classifies one leaf or dispatches to `inferArray` / `inferObject`.                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `inferArray`           | function  | array + depth + breadth + `closed` + ancestors → `JSONSchema` — empty array infers `{ type: 'array' }` with no `items`; otherwise samples up to `breadth` elements and unifies their inferred schemas into `items`.                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `inferObject`          | function  | record + depth + breadth + `closed` + ancestors → `JSONSchema` — sorted, breadth-capped own keys; a property read via `attempt` (a hostile getter or an `undefined` value is treated as ABSENT); every other present key is `required`.                                                                                                                                                                                                                                                                                                                                                                                                  |
| `inferRecordSamples`   | function  | plain-record samples + depth + breadth + `closed` → `JSONSchema` — the record-specialized branch `samplesToSchema` delegates to: unions keys, requires a key iff present in every sample, and recurses per-key values back through `samplesToSchema`.                                                                                                                                                                                                                                                                                                                                                                                    |
| `ValueToSchemaOptions` | interface | `{ maxDepth?, maxProperties?, closed? }` — `maxDepth` / `maxProperties` default to `INFER_DEPTH_LIMIT` / `INFER_BREADTH_LIMIT`; `closed` defaults to `true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
> **MCP caveat.** A non-object root schema — `valueToSchema('hello')` infers `{ type: 'string' }` — is structurally accepted by `schemaToParameters` (any `JSONSchema` is a record), but MCP clients expect an object-shaped `inputSchema`. Wrap a non-object payload in an object (e.g. `valueToSchema({ value: payload })`) before advertising it as a tool's parameters.
...
- [`tests/src/core/inferers.test.ts`](../../tests/src/core/inferers.test.ts) — `valueToSchema` / `samplesToSchema` leaf classification, `compileSchema` round-trip parity, heterogeneous-array `anyOf` unification, cycle/depth/breadth bounds, determinism, and the `schemaToParameters(valueToSchema(...))` seam.
