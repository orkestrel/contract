...
| API                    | Kind      | Summary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `valueToSchema`        | function  | value + optional `ValueToSchemaOptions` → `JSONSchema`. Total: never throws, cycle/depth/breadth-bounded. Leaf order: `null`, boolean, integer (`Number.isInteger` semantics — `-0` counts), finite non-integer number, non-finite number (widened to `{ type: 'number' }`), string (gains `format` when the `format` option is on and `stringToFormat` matches), array (recurse), plain record (recurse), `Date` (`{ type: 'string' }`, plus `format: 'date-time'` when `format` is on); anything else (function, symbol, bigint, `undefined`, `Map`/`Set`, …) → the empty accept-anything schema `{}`. Nested objects close to unknown keys by default (`additionalProperties: false`); pass `closed: false` to open them. Deterministic — sorted property keys and sorted `anyOf` members.                     |
| `samplesToSchema`      | function  | samples (`readonly unknown[]`) + optional `ValueToSchemaOptions` → `JSONSchema` — multi-example inference (e.g. several database rows). An empty array infers `{}`. When every sample is a plain record, a key is `required` only when present (and non-`undefined`) in EVERY sample, and `properties` is the union of every sample's keys; the per-key slot unifies across all samples' values for that key via `inferSamples`. Otherwise the whole sample list is inferred as one slot via `inferSamples`. `format` (default `false`) reattaches a unanimous `stringToFormat` result to a slot that resolves to bare `{ type: 'string' }`; `enum` (default `false`) lets a low-cardinality, repeated primitive slot emit `{ enum: [...] }` instead (enum wins over format when both fire — see `inferSamples`). |
...
| `unifySchemas`         | function  | `readonly JSONSchema[]` → `JSONSchema` — de-duplicates by canonical key, collapses a bare `{ type: 'integer' }` alongside a bare `{ type: 'number' }` into just `{ type: 'number' }`, and returns a single surviving distinct schema directly or `{ anyOf: [...] }` (sorted) for two-or-more. An empty list returns `{}`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `canonicalStringify`   | function  | `unknown` → `string` — a deterministic, key-sorted JSON encoding (pure ECMAScript, no `structuredClone`) backing `unifySchemas`'s de-duplication and ordering; two structurally-equal fragments built independently canonicalize identically.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `inferValue`           | function  | value + depth + breadth + `closed` + `format` + a `WeakSet` ancestor set → `JSONSchema` — the recursive spine `valueToSchema` and the container inferers share; classifies one leaf (string/`Date` gain `format` when the flag is on) or dispatches to `inferArray` / `inferObject`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `inferArray`           | function  | array + depth + breadth + `closed` + `format` + ancestors → `JSONSchema` — empty array infers `{ type: 'array' }` with no `items`; otherwise samples up to `breadth` elements and unifies their inferred schemas into `items`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `inferObject`          | function  | record + depth + breadth + `closed` + `format` + ancestors → `JSONSchema` — sorted, breadth-capped own keys; a property read via `attempt` (a hostile getter or an `undefined` value is treated as ABSENT); every other present key is `required`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `inferRecordSamples`   | function  | plain-record samples + depth + breadth + `closed` + `format` + `enumOn` → `JSONSchema` — the record-specialized branch `samplesToSchema` delegates to: unions keys, requires a key iff present in every sample, and recurses per-key values back through `inferSamples`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `inferPrimitiveEnum`   | function  | slot values + a cardinality limit → `{ enum: [...] } \| undefined` — fires only for a single-primitive-kind (all string or all number), repeated (distinct count < value count), low-cardinality (distinct count ≤ limit) slot; emits `enum` with NO `type` key, byte-matching `compileSchema`'s `literalShape` emission. Members sorted by canonical key.                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `stringToFormat`       | function  | `string` → `SchemaFormat \| undefined` — fixed precedence `uuid` > `date-time` > `date` > `time` > `email` > `uri`, first match wins. `date-time` / `date` / `time` require both an ISO-8601 shape match and a real `isValidISOInstant` validity check (`2020-13-45` is rejected). Total, pure, deterministic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `ValueToSchemaOptions` | interface | `{ maxDepth?, maxProperties?, closed?, format?, enum? }` — `maxDepth` / `maxProperties` default to `INFER_DEPTH_LIMIT` / `INFER_BREADTH_LIMIT`; `closed` defaults to `true`; `format` and `enum` default to `false`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- [`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) — `enumerableSymbolCount`, `seededRandom`, `schemaToParameters` (a compiled-contract record schema passes through by reference; a non-record yields `undefined`), and `schemaToObject` (object pass-through, every non-object root kind wrapped, determinism, the `schemaToParameters(schemaToObject(valueToSchema(...)))` seam).
...
- [`tests/src/core/inferers.test.ts`](../../tests/src/core/inferers.test.ts) — `valueToSchema` / `samplesToSchema` leaf classification, `compileSchema` round-trip parity, heterogeneous-array `anyOf` unification, cycle/depth/breadth bounds, determinism, and the `schemaToParameters(valueToSchema(...))` seam; `stringToFormat` / `samplesToFormat` per-format positive/negative/almost cases and unanimity; `format`/`enum` precedence and the multi-sample format-disabling seam (unanimous → format, disagreement → bare string, never `anyOf` of formats); enum cardinality/repetition eligibility and `literalShape` round-trip parity; `Date` inference (`{ type: 'string' }` default, `format: 'date-time'` opt-in) and exotic-value totality (`Map`/`Set`/`bigint`/typed array/function/symbol/`undefined` still `{}`); hostile-input containment with `format`/`enum` enabled.
